...
int main(int argc, char * argv[])
{
    ...
    double r, g, V=1.0,lambdaMax,lambdaMin, cond;
    ...
    VEC b(node*node),ans(node*node),temp(node*node),q1(node*node),q2(node*node);
    for(int i = 0; i < a.dim(); i++) q1[i]=q2[i] = 1;
    ...
    */
    ...
    lambdaMax = powerMethod(a,q1,5000);
    lambdaMin = invPowerShift(a,0.0,q2,5000);
    
    cond = lambdaMax/lambdaMin;
    printf("Lambda Max: %e\n",lambdaMax);
    printf("Lambda Min: %e\n",lambdaMin);
